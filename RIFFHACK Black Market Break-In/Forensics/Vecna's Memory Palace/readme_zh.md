...
首先可以看到 clockface 13，感覺就是做 ROT13，把所有東西都試過以後發現 `UNJX` -> `HAWK`，`VAF` -> `INS`，所以說其中某個東西是 `HAWKINS`

接著我把 memfrag 按照順序排好，b64 decode 以後按照 `POS` 的順序排序，至於 noise 和 cache_shadow 都沒屁用，純搞人而已
...
排好以後把上面的東西合起來對 `HAWKINS` 做 XOR以後會得到一串 JSON
