...
Initially, attempted login with admin and SQL injection techniques but was unsuccessful. Then visiting a random page displayed 404 Not Found. Upon opening the developer tools, found a comment:
...
After navigating to the path, discovered the following users file:
...
Noticed that the EAdmin password hash looked familiar—consisting entirely of digits. Referencing [this website](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Type%20Juggling/README.md#magic-hashes) for SHA-256 magic hashes, identified the password as 34250003024812.

After logging in with EAdmin credentials, changed the role to admin and visited the /admin endpoint to retrieve the flag.
